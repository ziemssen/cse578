...
cat_columns = [n for n in df.columns[:-1] if ((df[n].dtype==object))][:-1]
...
xst = pd.DataFrame.sparse.from_spmatrix(pipeline.fit_transform(X_train))

model.fit(xst, Y_train)

# Evaluate the model
xstt = pd.DataFrame.sparse.from_spmatrix(pipeline.transform(X_test))
score = model.score(xstt, Y_test)
print(f"Test score: {score}")

cols = pipeline['preprocessor'].transformers_[0][1].get_feature_names_out().tolist() +\
    pipeline['preprocessor'].transformers_[1][1].get_feature_names_out().tolist()
